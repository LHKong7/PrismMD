...
PrismMD 是一款跨平台桌面应用，将 Markdown 视为一等思考媒介——它是一个**个人知识库**，而不是一堆文件：笔记之间用 `[[双链]]` 相互连接，每篇笔记都带着自己的反向链接与相关笔记，整个工作区中英文都可检索、可提问，全部在本机完成。它不仅能渲染 GFM、LaTeX 和 Mermaid 图表，还内置 AI 阅读助手，可以与当前文档对话、记忆历史对话，并可选地从你保存的每篇文档构建的本地知识图谱中检索上下文。
