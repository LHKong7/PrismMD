<div align="center">
  ...
  <img src="assets/main_scene.png" alt="烛笺阁 / Inkwell Keep — 像素写作世界" width="760" />
</div>

---

PrismMD 是一款跨平台桌面应用，将 Markdown 视为一等思考媒介——它是一个**个人知识库**，而不是一堆文件：笔记之间用 `[[双链]]` 相互连接，每篇笔记都带着自己的反向链接与相关笔记，整个工作区中英文都可检索、可提问，全部在本机完成。它不仅能渲染 GFM、LaTeX 和 Mermaid 图表，还内置 AI 阅读助手，可以与当前文档对话、记忆历史对话，并可选地从你保存的每篇文档构建的本地知识图谱中检索上下文。而当你想**写作**时，推开那扇门，便走进 **烛笺阁 / Inkwell Keep** —— 一个 2D 像素风写作世界：你的文章是书架上的一本本书，一群 AI **贤者**陪你起草、会诊、打磨。

## 目录

- [烛笺阁 — 像素写作世界](#烛笺阁--像素写作世界)
- [核心功能](#核心功能)
- [把笔记变成知识库](#把笔记变成知识库)
- [环境要求](#环境要求)
- [快速上手](#快速上手)
...
PrismMD 有两副面孔。安静的**工作台**（你熟悉的阅读器 / 编辑器 / AI 助手）藏在一个轻松的**前台**之后：**烛笺阁 / Inkwell Keep** —— 启动即进入的 2D 像素风写作世界。用 **WASD / 方向键 / 点击** 操控一位小作家走动，每个房间就是写作流程的一环。

<table>
  <tr>
    <td width="50%"><img src="assets/the_scribes.png" alt="工位廊 —— 八位 AI 贤者" /></td>
    <td width="50%"><img src="assets/round_table.png" alt="写作圆桌 —— 多 Agent 会诊" /></td>
  </tr>
  <tr>
    <td align="center"><em>阁中诸贤 / 工位廊 —— 八位 AI 贤者</em></td>
    <td align="center"><em>写作圆桌 —— 多位贤者一起会诊一篇稿子</em></td>
  </tr>
</table>

- **典藏书柜** —— 每篇文章都是书架上的一本书，书脊反映真实元数据：长度 → 厚薄、类型 → 配色与材质、品质 → 烫金、状态 → 草稿半透明 / 热门发光 / 待修红「!」。点书即可阅读或编辑。
- **阁中诸贤** —— 八位 NPC 写作 Agent，各有专长与人设：结构、标题、观点、语言润色（降 AI 味）、读者视角、SEO/传播、技术审核、风格模仿。走过去即可对话，每位用你配置的 AI 服务、各自独立人设。
- **写作圆桌** —— 邀请多位贤者**同时**会诊当前文档；一位「主持」Agent 把意见去重合并成按优先级排好的清单，勾选即可生成新版本。
- **档案柜** —— 完整版本史，逐行 diff + 一键可逆回滚（圆桌改写会自动留存改前版本）。
- **灵感墙** —— 收集选题、金句、素材与开头候选，手动添加或让 AI 据当前稿子生成。

整套世界由 [Pixi.js](https://pixijs.com/) 渲染 —— 纯程序化像素美术，无需游戏素材。想用纯净工作台？右上角 **进入工作台** 按钮直接回到经典阅读器。
...
- **双链笔记** — 任意位置写 `[[笔记标题]]` 建立链接，输入 `[[` 即有标题补全
- **反向链接** — 每篇笔记都能看到「谁链接了我」，附带链接处的上下文片段
- **相关笔记** — 按链接、标签、用词三路信号融合推荐，并说明推荐理由
- **未写的链接** — 指向尚不存在的笔记不是错误，而是待写清单，点击即创建
- **重命名不断链** — 改标题时自动改写所有指向它的 `[[链接]]` 原文
- **中英文全文检索** — 按标题切块 + SQLite FTS5 + BM25 排序；中文按重叠 bigram 索引，
  不依赖任何外部服务
- **AI 引用你的笔记** — 助手检索全部笔记的**当前内容**并按 `[1]` `[2]` 标注出处，
  点击引用直接跳到那篇笔记的对应段落
...
PrismMD 会自动为工作区里的每一篇笔记建立本地索引——不需要开关，也不需要手动添加。
由此得到三件事：

**链接。** 任意位置输入 `[[` 选一篇笔记，它就渲染成可点击的链接，目标笔记随之
获得一条**反向链接**。指向尚不存在的笔记不是错误，而会显示成「尚未写过」，
点击即创建。改笔记标题时，所有指向它的 `[[链接]]` 会被就地改写——
改进标题不会让你失去连接。

**搜索。** 笔记按标题切成段落，用 SQLite FTS5 建索引，BM25 在正文、标题、
`#标签` 和链接图之间综合排序。**中英文都能搜**：中文按重叠 bigram 索引，
因为 SQLite 自带分词器会把整句中文当成一个词，结果是静默地什么都搜不到。

**带出处的回答。** 向助手提问时，它会检索你笔记中的相关段落，并被要求以
`[1]`、`[2]` 标注出处。每个引用都可点击：直接打开该段落所在的笔记并滚动到位。
这一切对任何服务商都成立，包括本地 Ollama，也不需要任何外部服务。

右侧栏的 **知识** 标签页展示当前笔记的位置：谁链接到这里、它链向哪里、
哪些笔记与之相关（按链接 / 标签 / 用词）、以及它的标签。
**设置 → 知识库** 里可以看到索引状态并手动重建。

> 另一项 [知识图谱](#知识图谱可选) 功能是**不同的、可选的**东西：
> 它把*实体*抽取到你自己运行的 Neo4j 里。上面的笔记索引完全不需要这些。

### 笔记即文件（可选）

默认情况下笔记存放在应用的数据库里。**设置 → 存储** 可以把它们迁移成
**Markdown 仓库**：一个装满普通 `.md` 文件的文件夹，可以用访达、git、Obsidian
或任何编辑器打开——而 PrismMD 的用法完全不变。

```text
Vault/
├── Projects/PrismMD.md      一篇笔记，frontmatter 里带稳定 id
├── Attachments/diagram.pdf  文档本身
├── .trash/                  已删除的笔记，可恢复
└── .prism/                  应用数据
    ├── ui.json              侧边栏顺序与图标 —— 可丢
    ├── binaries.json        无法自带 id 的文档的身份 —— 需备份
    └── annotations/         你的高亮 —— 需备份
```

迁移会把每一篇笔记复制到你选择的文件夹，先备份当前工作区，
并在切换前**逐篇校验**；只要有一篇对不上，就什么都不会切换，
未完成的副本会保留供你查看。

进入仓库模式后，在任何地方做的编辑都会被实时同步。在访达里改名或移动过的笔记
仍然是同一篇——id 在文件里，反向链接和高亮跟着它走。
如果笔记在磁盘上变化时你正在编辑，未保存的内容优先。

> `.prism/` 下有两样东西不是缓存：`binaries.json` 和 `annotations/`。
> 备份笔记时请一并带上；`ui.json` 丢了只是侧边栏退化成按名排序。
...
│   │   ├── knowledgeHandlers.ts   # 笔记索引：搜索 / 链接图 / 重建
│   │   └── ...                    # 标注、主题、插件、MCP 等
│   ├── knowledge/                 # 笔记索引：分词 / 切块 / 链接解析 / 排序 /
│   │                              #   SQLite+FTS5 引擎（纯逻辑，有单测）
│   └── services/                  # 主进程服务
│       ├── aiService.ts           # AI Agent（borderless-agent）
│       ├── memoryService.ts       # 对话记忆
│       ├── knowledgeService.ts    # 笔记索引调度 + 对外读取接口
...
│   ├── components/                # UI 组件
│   │   ├── agent/                 # AI 聊天侧边栏
│   │   ├── frontstage/            # 烛笺阁 像素世界（Pixi.js）：房间 / 贤者 / 圆桌 / 档案柜 / 书架
│   │   ├── reader/                # Markdown 渲染器 + 文档阅读
│   │   ├── editor/                # CodeMirror 编辑器
│   │   ├── knowledge/             # 双链渲染 + 知识面板（反向链接 / 相关笔记）
│   │   ├── graph/                 # 知识图谱可视化
│   │   ├── filetree/              # 文件资源管理器
│   │   ├── toc/                   # 目录导航
│   │   ├── annotations/           # 高亮标注
...
| [Pixi.js](https://pixijs.com/) | 驱动「烛笺阁」像素前台的 WebGL 渲染器 |
